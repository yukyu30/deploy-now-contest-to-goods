...
- Lambdaでの画像書き出し負荷を抑えるため、撮影時にJPEG（品質90）へ直接出力します。商品作成リクエストが15,000バイト以内に収まるよう、応答前にJPEG品質と解像度を段階的に下げます（本番CloudFrontで約16KBは通過、約32KBは403となることを確認）。縮小後の画像に署名し、プレビューと入稿に同じ画像を使います。入稿時は1732px角のPNGに拡大してQRコードを合成するため、サイト内の細かい文字は不鮮明になることがあります。
